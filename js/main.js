...
function toggleWishlist(productId) {
    ...
    if (index === -1) {
        // Add
        wishlist.push(productId);
        showToast(`تمت إضافة "${product ? product.name : 'المنتج'}" للمفضلة ❤️`);
    } else {
        // Remove
        wishlist.splice(index, 1);
        showToast(`تم حذف "${product ? product.name : 'المنتج'}" من المفضلة 💔`, 'info');
    }
...
function updateWishlistButtonState(productId) {
    ...
    // Find buttons targeting this product (requires us to tag them or find by context)
    // For specific product page button:
    const pageButtons = document.querySelectorAll(`.wishlist-btn[data-id="${productId}"]`);
